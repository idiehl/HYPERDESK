...
class AppController:
    def __init__(self, state) -> None:
        ...
        self._peer_checksums: dict[str, str] = {}
        self._bandwidth_history: list[tuple[float, float, float | None]] = []
        self._last_bandwidth_sample: float = 0.0
    ...
    async def _handle_control_message(self, message: dict) -> None:
        ...
        if message_type == "PAIRING_REQUEST":
            ...
            if job.status in ("complete", "failed", "skipped"):
                request_id = self._job_request_map.pop(job_id, None)
                if request_id:
                    mapped_status = "completed" if job.status == "complete" else job.status
    ...
    def _start_transfer(
        ...
        if request_id:
            self._job_request_map[job.id] = request_id
        self.state.update_transfer(job)
        self.storage.record_transfer(self.state.session.id, job)
    ...
    def _run_transfer_job(
        ...
        try:
            ...
            if network_transfer:
                self._record_expected_checksum(job.id, result.checksum)
            elif request_id:
    ...
    def _record_expected_checksum(self, job_id: str, checksum: str) -> None:
        if not checksum:
            return
        self._expected_checksums[job_id] = checksum
        peer_checksum = self._peer_checksums.pop(job_id, None)
        if peer_checksum:
            self._compare_checksums(job_id, checksum, peer_checksum)

    def _record_peer_checksum(self, job_id: str, checksum: str) -> None:
        if not checksum:
            return
        expected = self._expected_checksums.pop(job_id, None)
        if expected:
            self._compare_checksums(job_id, expected, checksum)
        else:
            self._peer_checksums[job_id] = checksum

    def _compare_checksums(self, job_id: str, expected: str, received: str) -> None:
        if expected == received:
            self.state.add_log(f"Checksum verified for job {job_id[:8]}.")
        else:
            self.state.add_log(
                f"Checksum mismatch for job {job_id[:8]} expected={expected[:8]} got={received[:8]}"
            )
    ...
    def record_bandwidth_sample(self, avg_rate_mbps: float, limit_mbps: float | None) -> None:
        now = time.time()
        if now - self._last_bandwidth_sample < 1.0:
            return
        self._last_bandwidth_sample = now
        self._bandwidth_history.append((now, avg_rate_mbps, limit_mbps))
        if len(self._bandwidth_history) > 300:
            self._bandwidth_history = self._bandwidth_history[-300:]

    def get_bandwidth_history(self) -> list[tuple[float, float, float | None]]:
